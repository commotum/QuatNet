...
def vector_to_rgb(P, d):
    """
    Map a 3D lattice point to an RGB color string based on normalized axis directions.
    
    Args:
        P (tuple of ints): (x, y, z)
        d (int): Lattice dimension
    
    Returns:
        str: RGB hex color string, like "#FF00AA"
    """
    if np.linalg.norm(P) == 0:
        return "#000000"  # Origin → black

    # Normalize P to unit vector
    V = np.array(P) / np.linalg.norm(P)
    
    # Map from [-1, 1] → [0, 255]
    rgb = ((V + 1) / 2 * 255).astype(int)
